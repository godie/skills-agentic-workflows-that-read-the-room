...
5. Following Mona's notes, identify useful, current updates that help developers learn GitHub faster. Keep summaries short and practical, and mention whether each update came from the GitHub Blog, GitHub Changelog, or Awesome Copilot workflows with its source URL.
6. Edit `site/content/github-info.md` with the curated updates. Preserve the file's existing structure and avoid unrelated changes.
7. Request the `create-pull-request` safe output with a concise title and body summarizing the updates and linking to every source. The pull request must propose the changes for Mona to review and must not write directly to the `main` branch.
